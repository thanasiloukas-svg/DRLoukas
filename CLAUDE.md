...
- **Pending deletion phase (AFTER owner runs a full Duplicator backup):** delete deactivated plugins above + elementor + elementor-pro + wp-super-cache + monsterinsights + asset-cleanup + 6 twenty* themes (keep one fallback; old loukas theme stays per rules); sweep stale Elementor meta. OptinMonster awaiting owner answer (runs popups?). Contact page 91 says "contact by online form" but NO form exists on it — decide: reword or add a form. WP 7.0.4→7.1 core update also waits for the backup.
...
- **GSC videos (35 not indexed)**: 30 "video isn't on a watch page" (the orphan mp4s — schema confirmed NOT the blocker), 5 "no thumbnail". Fix path: exclude raw mp4/attachment URLs from video sitemap or build watch pages; check what video-sitemap.xml (20 URLs) actually lists.
